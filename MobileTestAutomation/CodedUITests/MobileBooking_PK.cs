...
namespace MobileTestAutomation
{
    ...
    /// </summary>
    [CodedUITest]
    public class PK_MobileBooking:logWriter
    {
        public PK_MobileBooking()
        {
        }

        [TestMethod]
        public void MobileBooking_PK()
        {
            ...
            string flightExtras = string.Empty;
...
//runscript.OpenBrowser_Obj.locate();
            //runscript.verifyFExtrasDetails_obj.verifyTotalPriceonFltsPage();
            ...
            do
            {
                do
                {
                    ...
                    #region To be used this block of CODE was used to select a default date, Can be deleted later
            ...
            #region Search Results Page
				...
				VillaName = runscript.verifyVillaName_obj.verifyVillaName_Method();
                IsPKBtnthere = runscript.clickBookVillaAndFlightsBtn_obj.clickBookVillaAndFlightsBtn_Method(strURL);
            } while (IsPKBtnthere == false);
            #endregion
            ...
            string ATOLTxtFrmFltsPage = runscript.VerifyATOL_obj.VerifyATOL_Method();
            ...
            runscript.verifyViewSummaryOpened_obj.verifyFltSectioninViewSummary_Method(FlightDetailsfromPage, ATOLTxtFrmFltsPage);
            ...
            Dictionary<string, string> ListofFExtrasFrmDB = new Dictionary<string, string>();
            sqlQuery = "SELECT Invoicedesc,unitprice FROM [Booking].[ExtraBooked] WHERE Type = 'F' and Lineid in (SELECT  id FROM [Booking].[VillaPack] WHERE bookid=" + BookingID + ")";

            dataReader = getData.getData(sqlQuery, InitialCatalog);
            while (dataReader.Read())
            {
                ListofFExtrasFrmDB.Add(dataReader[0].ToString(), dataReader[1].ToString());
            }
            Tuple<bool, string> FExtrasValues = runscript.verifyDftFExtras_obj.verifyDftFExtras_Method();
            if (FExtrasValues.Item1 == true && FExtrascount > 0)
            {
                runscript.clickScrollArrow_obj.clickScrollArrow_Method();
                runscript.verifyFExtrasDetails_obj.verifyFExtrasDetails_Method(ListofFExtrasFrmDB, strURL);
            }
            ...
            //string NewFlightInfo = runscript.selectFlightChckBox_obj.selectFlightChckBox_Method(strURL);
            //runscript.clickSelectFlightBtn_obj.clickSelectFlightBtn_Method(strURL);
            //runscript.verifyFlightsPageOpened_obj.verifyFlightsPageOpened_Method(strURL);
            ////Need to verify the Flight is updated properly by comparing with NewFlightInfo details.
            ////*****Retriveing Flight Extras count and Details from DB After Amending Flights*****//

            ListofFExtrasFrmDB = new Dictionary<string, string>();
            FExtrascount = 0;
            string sqlQuery1 = "SELECT count(*) FROM [Booking].[ExtraBooked] WHERE Type = 'F' and Lineid in (SELECT  id FROM [Booking].[VillaPack] WHERE bookid=" + BookingID + ")";

            dataReader = getData.getData(sqlQuery1, InitialCatalog);
            while (dataReader.Read())
            {
                FExtrascount = Convert.ToInt32(dataReader[0].ToString());
            }
            string sqlQuery2 = "SELECT Invoicedesc,unitprice FROM [Booking].[ExtraBooked] WHERE Type = 'F' and Lineid in (SELECT  id FROM [Booking].[VillaPack] WHERE bookid=" + BookingID + ")";

            dataReader = getData.getData(sqlQuery2, InitialCatalog);
            while (dataReader.Read())
            {
                ListofFExtrasFrmDB.Add(dataReader[0].ToString(), dataReader[1].ToString());
            }
            FExtrasValues = runscript.verifyDftFExtras_obj.verifyDftFExtras_Method();
            if (FExtrasValues.Item1 == true && FExtrascount > 0)
            {
                runscript.clickScrollArrow_obj.clickScrollArrow_Method();
                runscript.verifyFExtrasDetails_obj.verifyFExtrasDetails_Method(ListofFExtrasFrmDB, strURL);
            ...
            FExtrasValues = runscript.verifyDftFExtras_obj.verifyDftFExtras_Method();
            FlightDetailsfromPage = runscript.verifyDftFlightDetails_obj.verifyDftFlightDetails_Method(strURL);
            runscript.clickViewSummary_obj.clickViewSummary_Method();
            runscript.verifyViewSummaryOpened_obj.verifyViewSummaryOpened_Method();
            runscript.verifyViewSummaryOpened_obj.verifyFltSectioninViewSummary_Method(FlightDetailsfromPage, ATOLTxtFrmFltsPage);
            runscript.clickHideSummary_obj.clickHideSummary_Method();
            ...
            runscript.clickViewSummary_obj.clickViewSummary_Method();
            runscript.verifyViewSummaryOpened_obj.verifyViewSummaryOpened_Method();
            runscript.verifyViewSummaryOpened_obj.verifyFltSectioninViewSummaryFrmGDPge_Method(FExtrasValues.Item2);
            runscript.clickHideSummary_obj.clickHideSummary_Method();
            ...
            string VExtrasDetailsFrmPge = runscript.verifyVillaExtrasAvailable_obj.verifySelectedVillaExtras_Method();
            ...
            //runscript.verifyCarsCategory_obj.verifyCarsCategory_Method(CentreName, AirportCodeName, SelectYear, selectDate);
            runscript.clickAddCarHireButton_obj.clickAddCarHireButton_Method();
            string selectedCar = runscript.verifyCarHireCategory_obj.verifyCarHireCategory_Method(CentreName, AirportCodeName, SelectYear, selectDate, InitialCatalog);
            ...
            runscript.verifyThankUPageOpened_obj.VerifyAllSectionsInThankYouPage_Method(BookingType, FlightDetailsfromPage, ATOLTxtFrmFltsPage, FExtrasValues.Item2, VExtrasDetailsFrmPge, CarHireDetailsFrmPge, TransferDetailsFrmPge, ADWDetailsFrmPge, LeadPaxDetailsFrmPge, OtherPaxDetailsFrmPge);
            #endregion
